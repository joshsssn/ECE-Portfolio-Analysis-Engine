...
class TimeSeriesDataset_SingleCSV_Inference(Dataset):
    ...
    def __init__(
        ...
        # ---- Drop junk columns (e.g. trailing 'Unnamed:' columns from Refinitiv CSVs)
        ...
        # Optional row-wise NaN drop — ONLY on columns we actually use
        # (prevents irrelevant all-NaN columns like Volume from wiping all rows)
        if self.dropna:
            df = df.dropna(subset=use_cols, axis=0).reset_index(drop=True)
